...
MODEL_NAME="full-staff-faster-rcnn-inception-resnet-v2-resnet101"
...
echo "Experiment: Full Staff Detection with Faster R-CNN + resnet101 with Inception ResNet v2 parameters" | tee -a "$LOG_FILE"
echo "Model name: $MODEL_NAME" | tee -a "$LOG_FILE"
echo "Output directory: $OUTPUT_DIR" | tee -a "$LOG_FILE"
echo "Log directory: $LOG_DIR" | tee -a "$LOG_FILE"
echo "Log file: $LOG_FILE" | tee -a "$LOG_FILE"

# Run the training script with configurations adapted for Inception ResNet v2 and resnet101
echo -e "\n==== Starting Training ====" | tee -a "$LOG_FILE"
echo "Command executed:" | tee -a "$LOG_FILE"

# Construct the full command with parameters
python3 /homes/es314/omr-objdet-benchmark/scripts/faster_rcnn/inception_resnetv2_resnet101_training.py \
  --data_dir=/homes/es314/omr-objdet-benchmark/data/staff_faster_rcnn_prepared_dataset \
  --output_dir=/import/c4dm-05/elona/faster-rcnn-models-march-2025/full-staff-faster-rcnn-inception_resnet_v2-resnet101 \
  --log_dir=/import/c4dm-05/elona/faster-rcnn-models-march-2025/full-staff-faster-rcnn-inception_resnet_v2-resnet101-logs \
