...
def train_model(model, train_loader, val_loader, device, 
               ...
               start_epoch=0, best_val_loss=float('inf'), history=None):
    """
    ...
    Args:
        ...
        start_epoch (int): Starting epoch (for continuation)
        best_val_loss (float): Best validation loss (for continuation)
        history (dict): Previous training history (for continuation)
...
def train_staff_line_model(img_dir, mask_dir=None, xml_dir=None, model_dir="models", 
                          ...
                          target_size=(512, 512), subset_fraction=1.0, continue_from=None):
    """
    ...
    Args:
        ...
        continue_from (str, optional): Path to checkpoint to continue training from
    ...
    # Initialize optimizer first (we'll reload state later if needed)
    ...
    if continue_from and os.path.exists(continue_from):
        ...
        checkpoint = torch.load(continue_from, map_location=device)
        
        # If it's a full checkpoint (not just weights)
        if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
            ...
            if 'optimizer_state_dict' in checkpoint:
                # Create a new optimizer with the right parameters
                optimizer = optim.Adam(model.parameters(), lr=lr)
                
                # Manually copy state to the new optimizer to ensure it's on the right device
                optimizer_state = checkpoint['optimizer_state_dict']
                
                # Make sure all tensors in optimizer state are on the correct device
            ...
            # Get best validation loss and history if available
            ...
            print(f"Resuming training from epoch {start_epoch} of {num_epochs}")
            print(f"Best validation loss so far: {best_val_loss:.4f}")
        else:
            # If it's just weights
            model.load_state_dict(checkpoint)
            model.to(device)
            print("Loaded model weights, but starting training from scratch")
    elif weights:
...
if __name__ == "__main__":
    ...
    if args.mode == "train":
        ...
        train_staff_line_model(
            ...
            continue_from=args.continue_from  # Added continue_from parameter
        )
