...
// Mock model data (as in your code)
const MOCK_MODEL_DATA = {
  "dual-model-pipeline": { // Add the dual-model option
    "imgsz": 1280,
    "epochs": "N/A",
    "batch": "N/A",
    "patience": "N/A",
    ...
    "pretrained": true,
    "optimizer": "N/A",
    "data": "dual-model-approach",
    "model": "Structure: Faster R-CNN, Symbol: YOLOv8",
    "description": "Combines Faster R-CNN for structure detection with YOLOv8 for symbol detection"
  },
  "train8": {
    "imgsz": 1024,
    "epochs": 120,
...
// ModelCard component
const ModelCard = ({ model, data, isSelected, onSelect }) => {
  // Special styling for dual-model-pipeline card
  const isDualModel = model === "dual-model-pipeline";
  ...
  return (
    <div
      className={`flex-shrink-0 w-60 p-2 border rounded-md cursor-pointer ${
        isSelected 
          ? 'border-indigo-500 bg-indigo-50' 
          : isDualModel ? 'border-purple-300 hover:border-purple-500 bg-purple-50' : 'border-gray-200 hover:border-indigo-300'
      }`}
      onClick={() => onSelect(model)}
    >
      <div className={`font-medium text-sm mb-1 truncate ${isDualModel ? 'text-purple-800' : ''}`}>
        {model}
        {isDualModel && <span className="ml-1 text-xs font-normal text-purple-600">NEW</span>}
      </div>
      <div className="text-xs text-gray-500">
        {isDualModel ? (
          <div className="text-purple-700 font-medium">
            <div>Structure: Faster R-CNN</div>
            <div>Symbols: YOLOv8</div>
            <div className="mt-1 text-xs italic">Advanced dual-model pipeline with MusicXML</div>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-x-2 gap-y-1">
            <div className="flex items-center gap-1">
...
const ModelDetails = ({ model, data }) => {
  ...
  if (isDualModel) {
    return (
      <div className="mb-4 bg-purple-50 p-3 rounded-md text-xs max-h-64 overflow-y-auto border border-purple-200">
        <h4 className="font-medium text-sm mb-2 text-purple-800">Dual-Model Pipeline</h4>
        
        <div className="space-y-3">
          {/* Dual Model Description */}
          <div>
            <h5 className="text-xs font-semibold text-purple-700 mb-1 border-b border-purple-200 pb-1">
              Pipeline Architecture
            </h5>
            <div className="text-purple-800 space-y-2">
              <p>This advanced pipeline uses two specialized models working together:</p>
              <div className="pl-2 border-l-2 border-purple-300">
                <div className="font-medium">Structure Model (Faster R-CNN)</div>
                <div className="text-xs pl-2 mb-1">Specialized in detecting structural elements like barlines, stems, and beams</div>
                <div className="font-medium mt-1">Symbol Model (YOLOv8)</div>
                <div className="text-xs pl-2">Specialized in detecting noteheads, clefs, accidentals, and other musical symbols</div>
              </div>
              <p>The two models' detections are combined with specialized rest handling, followed by staff line detection, 
                 pitch identification, and direct MusicXML generation.</p>
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="list-disc pl-4 text-green-800 space-y-1">
              <li>Higher accuracy for structural elements</li>
              <li>Better pitch identification</li>
              <li>Direct MusicXML generation</li>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-blue-800">
              <div className="flex justify-between">
                <span className="font-medium">Structure Model:</span>
                <span>Faster R-CNN ONNX</span>
              </div>
              <div className="flex justify-between">
                <span className="font-medium">Symbol Model:</span>
                <span>YOLOv8 PT</span>
              </div>
              <div className="flex justify-between">
                <span className="font-medium">Staff Detection:</span>
                <span>Pixel-perfect</span>
              </div>
              <div className="flex justify-between">
                <span className="font-medium">Stem Enhancement:</span>
                <span>Enabled</span>
              </div>
              <div className="flex justify-between">
...
const App = () => {
  ...
  const [detections, setDetections] = useState([]);
  const [pitchData, setPitchData] = useState(null);
  const [musicXMLContent, setMusicXMLContent] = useState(null); // Add state for MusicXML content
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState(null);
  
  // Model states
  const allModels = ["dual-model-pipeline", ...Object.keys(MOCK_MODEL_DATA).filter(key => key !== "dual-model-pipeline")];
  const [availableModels] = useState(allModels);
  const [selectedModel, setSelectedModel] = useState("dual-model-pipeline"); // Set dual model as default
  const [modelDetailsVisible, setModelDetailsVisible] = useState(true);
  const [modelCardsVisible, setModelCardsVisible] = useState(false);
  const [configVisible, setConfigVisible] = useState(false);
  ...
  const processImage = async (file, model) => {
    ...
    setIsLoading(true);
    setError(null);
    setMusicXMLContent(null); // Reset MusicXML content
    ...
    try {
      ...
      try {
        ...
        console.log('Pitch data present:', data.pitch_data ? 'Yes' : 'No');
        console.log('Base name:', data.base_name || 'Missing');
        console.log('MusicXML present:', data.musicxml ? 'Yes' : 'No');
        ...
        // Save MusicXML content if available directly from API
        if (data.musicxml) {
          console.log('MusicXML directly from API response');
          setMusicXMLContent(data.musicxml);
        }
        ...
        // If we don't have direct MusicXML content but we have the base_name,
        // try to fetch the MusicXML separately (for dual-model pipeline)
        if (!data.musicxml && data.base_name && model === "dual-model-pipeline") {
          console.log('Fetching MusicXML separately with base_name:', data.base_name);
          fetchMusicXML(data.base_name);
        }
  ...
  // Function to fetch MusicXML using base_name
  const fetchMusicXML = async (baseName) => {
    if (!baseName) return;
    
    try {
      console.log(`Fetching MusicXML for base_name: ${baseName}`);
      
      const response = await fetch('/convert_to_musicxml', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        console.error('Error fetching MusicXML:', errorText);
        return;
      }
      ...
      if (data.musicxml) {
        console.log('Successfully fetched MusicXML');
        setMusicXMLContent(data.musicxml);
        
        // Update the processing history to indicate we have MusicXML
        setProcessingHistory(prev => 
          prev.map(item => 
            item.baseName === baseName 
              ? { ...item, hasMusicXML: true } 
              : item
          )
        );
      }
              ...
              <div className="flex items-center gap-2 mb-3">
                <div className="flex-grow">
                  ...
                  >
                    {availableModels.map(model => (
                      <option key={model} value={model}>
                        {model} {model === "dual-model-pipeline" ? "(Recommended)" : ""}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-shrink-0">
                    <span className={`px-2 py-1 ${
                      selectedModel === "dual-model-pipeline" 
        ...
        {isLoading && (
          <div className="flex justify-center items-center my-4 p-2">
            <div className={`animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 ${
              selectedModel === "dual-model-pipeline" ? "border-purple-500" : "border-indigo-500"
            }`}></div>
            <span className={selectedModel === "dual-model-pipeline" ? "ml-3 text-purple-700" : "ml-3 text-indigo-700"}>
                  ...
                  {processingHistory.map((item, index) => (
                    <div 
                      ...
                      className={`flex-shrink-0 px-2 py-1 rounded-full ${
                        item.model === "dual-model-pipeline" 
                    ...
                    >
                      ...
                      <span>{item.detectionCount}</span>
                      {item.hasMusicXML && (
                        <span className="ml-1 text-xs bg-green-100 text-green-800 px-1 rounded">MusicXML</span>
                      )}
                    </div>
                  ))}
                </div>
        ...
        {/* Display Controls with Image Availability Indicators */}
          ...
          <div className="mb-4 p-3 bg-gray-100 rounded-md">
            ...
            <div className="grid grid-cols-4 gap-2 text-xs">
              ...
              <div className={`p-1 rounded ${musicXMLContent ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                MusicXML: {musicXMLContent ? '✓ Available' : '⚠ Not generated'}
              </div>
            </div>
          </div>
          ...
          */}
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {detections.slice(0, 20).map((detection, index) => {
                      try {
                        ...
                        if (detection.source_model) {
                          details.push(`Source: ${detection.source_model}`);
                        }
          ...
          {showMusicXML && (
            <div className="mb-4">
              <div className="bg-orange-100 px-3 py-2 border border-orange-300 rounded-t flex justify-between items-center">
                <h3 className="font-medium text-sm text-orange-700">MusicXML Notation Preview</h3>
                {musicXMLContent && (
                  <button
                    onClick={() => {
              ...
              <div className="border border-orange-300 border-t-0 rounded-b">
                <MusicXMLViewer 
                  pitchData={pitchData} 
                  baseName={processingHistory.length > 0 ? processingHistory[processingHistory.length - 1]?.baseName : null}
