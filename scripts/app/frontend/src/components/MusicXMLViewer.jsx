...
const MusicXMLViewer = ({ pitchData, baseName, musicXMLContent }) => {
  const [musicXML, setMusicXML] = useState(musicXMLContent);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [timeSignature, setTimeSignature] = useState('4/4');
  const [beamAll, setBeamAll] = useState(false);
  ...
  // Check for direct MusicXML content on component mount or when props change
  useEffect(() => {
    if (musicXMLContent) {
      setMusicXML(musicXMLContent);
    }
  }, [musicXMLContent]);
  
  const convertToMusicXML = async () => {
    // If we already have MusicXML content, no need to convert
    if (musicXML) return;
    ...
    try {
      // Try using baseName first if available (for dual-model pipeline)
      if (baseName) {
        console.log(`Fetching MusicXML using baseName: ${baseName}`);
        ...
        if (!response.ok) {
          console.warn(`Failed to fetch MusicXML with baseName, falling back to pitch data`);
          throw new Error('Failed to fetch MusicXML using baseName');
        }
      ...
      // Fall back to using pitch data if baseName doesn't work or isn't provided
      if (!pitchData) {
        setError('No pitch data available for conversion');
        return;
      }
      
      console.log('Using legacy conversion with pitch data');
      const response = await fetch('/api/convert_to_musicxml', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  // Auto-convert if we have baseName but no MusicXML
  useEffect(() => {
    if (!musicXML && (baseName || pitchData)) {
      convertToMusicXML();
    }
  }, []);
    ...
    <div className="music-xml-section bg-white p-6 rounded-lg shadow-md mb-6">
      ...
      <div className="bg-gray-50 p-4 rounded-md mb-4">
        ...
        <div className="flex gap-2">
          <button 
            onClick={convertToMusicXML} 
            disabled={loading || (!pitchData && !baseName) || musicXML}
            className={`px-4 py-2 rounded-md text-sm font-medium ${
              loading || (!pitchData && !baseName) || musicXML
                ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                : 'bg-indigo-600 text-white hover:bg-indigo-700'
            }`}
          >
            ...
            ) : musicXML ? 'MusicXML Ready' : 'Convert to MusicXML'}
          </button>
