...
from informed_staffline_detection_april import detect_and_save_staff_lines, merge_detections, AdaptiveStaffDetector
...
def process_single_image(image_path, img_stem, model_path, class_mapping_file,
    ...
    """Process a single image through the entire pipeline with improved staff detection"""
    import shutil
    import json
    import os
    ...
    # MODIFICATION: Changed the order - detect objects first to use them for staff detection
    
    # Step 1: Detect musical symbols first
    ...
    # Load detected music symbols if available
    detected_elements = None
    if os.path.exists(object_json):
        try:
            with open(object_json, 'r') as f:
                detected_elements = json.load(f)
                print(f"Loaded {len(detected_elements.get('detections', []))} detected elements")
        except Exception as e:
            print(f"Error loading detected elements: {e}")
    
    # Step 2: Now detect staff lines with the help of detected elements
    print("Step 2: Detecting staff lines (with musical element assistance)...")
    ...
    try:
        # Use the modified detect method with detected_elements
        staff_data = detector.detect(image_path, detected_elements)
        
        # If staff detection didn't find any staff systems but we have detected elements,
        # let's try to infer staff lines purely from the musical elements
        if not staff_data or len(staff_data.get("staff_systems", [])) == 0:
            if detected_elements and len(detected_elements.get("detections", [])) > 0:
                staff_data = detector._infer_from_musical_elements(
                    cv2.imread(image_path, cv2.IMREAD_GRAYSCALE),
                    detected_elements
                )
        
        # If we have some staff lines but not complete systems,
        # refine them using detected elements
        elif detected_elements and len(detected_elements.get("detections", [])) > 0:
            ...
            if incomplete_systems:
                ...
                staff_data = detector._refine_staff_systems(staff_data, detected_elements)
        
        # Save staff data
        detector.save_data(staff_data, staff_json)
        
        # Create visualization
        detector.visualize(image_path, staff_data, staff_viz)
    except Exception as e:
        print(f"Error in staff line detection: {e}")
