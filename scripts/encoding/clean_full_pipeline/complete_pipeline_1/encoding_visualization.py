# /homes/es314/omr-objdet-benchmark/scripts/encoding/clean_full_pipeline/complete_pipeline_1/generate_musicxml.py
...
def visualize_overlay(processor, image_path, output_path=None):
    ...
    # Fix beam visualization in visualize_overlay():
    for beam in processor.beams:
        draw_bbox(beam, colors['beam'])
        
        # Only draw connections to properly connected notes
        if hasattr(beam, 'connected_notes') and beam.connected_notes:
            # Sort notes left to right for cleaner visualization
            connected_notes = sorted(beam.connected_notes, key=lambda n: n.x)
            
            # Draw appropriate connections to all connected notes, regardless of distance
            for note in connected_notes:
                # Calculate beam y-position at this note's x-position
                # For a beam with endpoints (x1,y1) and (x2,y2), the y at position x is:
                ...
                if beam_x2 != beam_x1:
                    beam_slope = (beam_y2 - beam_y1) / (beam_x2 - beam_x1)
                    beam_y_at_note = beam_y1 + beam_slope * (note.x - beam_x1)
                else:
                    beam_y_at_note = beam.y  # Fallback
                
                # Draw a vertical line from the note to the beam
                ax.plot([note.x, note.x], [note.y, beam_y_at_note],
                        linestyle='-', color=colors['beam'], alpha=0.7)
    ...
    for note in processor.notes:
        if hasattr(note, 'is_chord_member') and note.is_chord_member and hasattr(note, 'chord') and note == note.chord[0]:
            # Get staff ID for this note
            note_staff_id = processor._get_staff_id(note) if hasattr(processor, '_get_staff_id') else None
            
            # Filter chord notes to include only those on the same staff
            same_staff_notes = [n for n in note.chord if 
                            processor._get_staff_id(n) == note_staff_id]
            
            # Only draw if we have at least 2 notes in the same staff
            if len(same_staff_notes) >= 2:
                chord_points = [(n.x, n.y) for n in same_staff_notes]
    ...
    for note in processor.notes:
        if hasattr(note, 'is_chord_member') and note.is_chord_member and hasattr(note, 'chord') and note == note.chord[0]:
            ...
            if len(same_staff_notes) >= 2:
                ...
                # Optionally add staff ID label
                ax.text(xs[0] - 15, ys[0], f"Staff {note_staff_id}",
                    fontsize=5, color=colors['chord'])
