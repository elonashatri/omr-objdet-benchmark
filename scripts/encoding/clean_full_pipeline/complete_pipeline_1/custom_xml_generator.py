...
try:
    from music_elements import Accidental, Note, Rest, Clef, Barline, TimeSignatureElement, Stem
except ImportError:
...
def generate_custom_xml(processor, output_path):
    ...
    try:
        ...
        # Process stems
        stem_count = 0
        processed_stems = set()  # Track stems we've already processed to avoid duplicates

        # First process stems from the processor's main stems list
        if hasattr(processor, 'stems'):
            print(f"Processing {len(processor.stems)} stems from processor.stems collection")
            for stem in processor.stems:
                node_elem = ET.SubElement(nodes_elem, "Node")
                ...
                # Find system for this stem (for order calculation)
                stem_system = None
                if hasattr(stem, 'staff_system') and stem.staff_system:
                    stem_system = stem.staff_system
                
                # Set order
                if stem_system:
                    base_order = len(stem_system.lines) + clef_count + key_sig_count + time_sig_count + barline_count + beam_count
                else:
                    # Default base order if no system found
                    base_order = clef_count + key_sig_count + time_sig_count + barline_count + beam_count
                
                order_val = base_order + stem_count + 1
                node_elem.set("order_id", str(order_val))
                stem_count += 1
                
                # Track this stem to avoid duplicates
                processed_stems.add(stem)

        # Then process any additional stems in staff systems that weren't in the main stems list
        for system in processor.staff_systems:
            stems = [e for e in system.elements 
                    if (hasattr(e, 'class_name') and 'stem' in e.class_name.lower()) or
                        (isinstance(e, Stem)) 
                    ]
            ...
            for stem in stems:
                ...
                # Track this stem to avoid duplicates next time
                processed_stems.add(stem)
        ...
        for note in processor.notes:
            ...
            if hasattr(note, 'step') and hasattr(note, 'octave') and note.octave is not None:
                ...
                add_data_item(data_elem, "pitch_step", note.step, "str")  # Add raw step value
                
                # Add accidental/alter value explicitly
                if hasattr(note, 'alter') and note.alter is not None:
                    add_data_item(data_elem, "pitch_alter", note.alter, "float")
                else:
                    add_data_item(data_elem, "pitch_alter", 0, "float")  # Default no alteration

                # Add staff position if available
                if hasattr(note, 'staff_position'):
                    add_data_item(data_elem, "staff_position", note.staff_position, "int")
                ...
                # Add full pitch string (e.g., "C4", "D#5")
                alter_symbol = ""
                ...
                normalized_pitch_step = f"{note.step}{alter_symbol}"
                add_data_item(data_elem, "normalized_pitch_step", normalized_pitch_step, "str")
            else:
                # Log warning if pitch information is incomplete
                print(f"Warning: Incomplete pitch information for note at ({note.x:.1f}, {note.y:.1f})")
