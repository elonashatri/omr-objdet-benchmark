...
def generate_custom_xml(processor, output_path):
    """
    Generate a comprehensive custom XML format capturing all music notation elements
    and their relationships from an OMRProcessor object.
    ...
    try:
        ...
        for system_idx, system in enumerate(processor.staff_systems):
            ...
            if hasattr(system, 'lines') and system.lines:
                ...
                for i, (line_num, y_pos) in enumerate(sorted_lines):
                    ...
                    add_data_item(data_elem, "ordered_staff_id", system_idx, "int")
        ...
        for system_idx, system in enumerate(processor.staff_systems):
            ...
            for clef_idx, clef in enumerate(clefs):
                ...
                if hasattr(system, 'spacing_run_id'):
                    add_data_item(data_elem, "spacing_run_id", system.spacing_run_id, "int")
                else:
                    add_data_item(data_elem, "spacing_run_id", spacing_run_id, "int")
        ...
        for rest in processor.rests:
            ...
            if hasattr(rest, 'staff_system') and rest.staff_system:
                ...
                # No staff system - use "unk" values
                print(f"  Warning: Rest at ({rest.x}, {rest.y}) has no staff system")
        ...
        for note in processor.notes:
            ...
            if hasattr(note, 'staff_system') and note.staff_system:
                ...
                # No staff system - use "unk" values
                print(f"  Warning: Note at ({note.x}, {note.y}) has no staff system")
                add_data_item(data_elem, "staff_id", "unk", "str")
                add_data_item(data_elem, "spacing_run_id", "unk", "str")
                add_data_item(data_elem, "ordered_staff_id", "unk", "str")
                
                # Use a default order value
        ...
        for artic in processor.articulations:
            ...
            if system:
                ...
                # No staff system - use "unk" values
                print(f"  Warning: Articulation at ({artic.x}, {artic.y}) has no staff system")
                add_data_item(data_elem, "staff_id", "unk", "str")
                add_data_item(data_elem, "spacing_run_id", "unk", "str")
                add_data_item(data_elem, "ordered_staff_id", "unk", "str")
        ...
        for dynamic in processor.dynamics:
            ...
            # Determine system
            system = None
            if hasattr(dynamic, 'measure') and dynamic.measure and hasattr(dynamic.measure, 'system'):
                system = dynamic.measure.system
            ...
            if system:
                ...
                # No staff system - use "unk" values
                print(f"  Warning: Dynamic at ({dynamic.x}, {dynamic.y}) has no staff system")
                add_data_item(data_elem, "staff_id", "unk", "str")
                add_data_item(data_elem, "spacing_run_id", "unk", "str")
                add_data_item(data_elem, "ordered_staff_id", "unk", "str")
            
            # Set order
            order_val = 3000 + dynamic_count + 1  # Use a base value for elements without a system
            if system:
                base_order = (len(system.lines) + clef_count + key_sig_count + time_sig_count + barline_count + 
                            beam_count + stem_count + flag_count + accidental_count + rest_count + note_count + 
        ...
        print(f"  Total dynamics processed: {dynamic_count}")
        

        # # Process gradual dynamics (crescendo/diminuendo) without requiring system
        # grad_dynamic_count = 0
        # print(f"\n=== PROCESSING GRADUAL DYNAMICS ===")
        # for grad in processor.gradual_dynamics:
        #     # Create node element regardless of system
        #     node_elem = ET.SubElement(nodes_elem, "Node")
        #     node_id = get_element_id(grad)
        #     node_elem.set("Id", str(node_id))
            
        #     # Set class name based on gradual dynamic type
        #     if hasattr(grad, 'type') and grad.type.lower() == 'crescendo':
        #         node_elem.set("ClassName", "crescendo")
        #     else:
        #         node_elem.set("ClassName", "diminuendo")
