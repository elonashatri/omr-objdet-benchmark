...
Usage:
    ...
    python complete_pipeline.py --image_dir /path/to/images 
                               --structure_model path/to/faster_rcnn.onnx 
                               --structure_mapping path/to/structure_classes.txt
                               --symbol_model path/to/yolo.pt 
                               --symbol_mapping path/to/symbol_classes.json
...
class SilenceStdout:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.silent:
            ...
            # If there was an exception, print the captured output
            if exc_type is not None:
                print(self.temp_stdout.getvalue())
...
def detect_structure_elements(image_path, model_path, class_mapping_path, output_dir, 
    ...
    try:
        ...
        detection_json = os.path.join(output_dir, f"{img_stem}_structure_detections.json")
...
def detect_music_symbols(image_path, model_path, class_mapping_path, output_dir, 
    ...
    try:
        ...
        # Get the path to the detection JSON file
        detection_json = os.path.join(output_dir, f"{img_stem}_symbol_detections.json")
...
#     """
#     Generate MusicXML from the merged detections, and also generate custom XML format
...
#         # Import debug function if available
#         try:
#             from debug_chord import debug_leftover_chord_attributes
#             debug_leftover_chord_attributes(processor)
#         except ImportError:
#             logger.warning("debug_chord module not available, skipping chord analysis")
...
# Also modify the process_image function to use SilenceStdout when calling individual processing functions
def process_image(image_path, structure_model_path, structure_mapping_path, 
                 ...
                 high_conf_threshold=0.5, overlap_threshold=0.5, silent=False):
    """
    ...
    Args:
        ...
        silent: Whether to silence standard output
    ...
    try:
        ...
        image_output_dir = os.path.join(output_base_dir)
        os.makedirs(image_output_dir, exist_ok=True)
        
        # Create output directories for each stage
        dirs = create_output_directories(image_output_dir)
        
        # Process using SilenceStdout context manager for each step
        with SilenceStdout(silent):
            ...
            # logger.info("Step 5: Generating MusicXML and custom XML...")
        ...
        result = {
            ...
            "merged": merged_path
            # "musicxml": xml_results["musicxml"] if xml_results else None,
            # "custom_xml": xml_results["custom_xml"] if xml_results else None,
            # "symbolic_viz": xml_results["symbolic_viz"] if xml_results else None,
            # "overlay_viz": xml_results["overlay_viz"] if xml_results else None
        }
...
def process_directory(image_dir, structure_model_path, structure_mapping_path, 
                     symbol_model_path, symbol_mapping_path, output_base_dir, 
                     conf_threshold=0.3, max_detections=1000, iou_threshold=0.35,
                     pixel_perfect=True, skip_enhancement=False,
                     high_conf_threshold=0.5, overlap_threshold=0.5, silent=False):
    ...
    try:
        ...
        for i, image_file in enumerate(sorted(image_files)):
            ...
            try:
                # Use the SilenceStdout context manager if silent is True
                with SilenceStdout(silent):
                ...
                # Print minimal progress info even in silent mode
                if silent:
                ...
                # Print error info even in silent mode
                if silent:
        ...
        logger.error(f"Error processing directory {image_dir}: {e}")
        logger.error(traceback.format_exc())
        return {"successful": [], "failed": []}
...
def main():
    ...
    # Required arguments - make image and image_dir optional but require at least one
    parser.add_argument("--image", help="Path to input image (single file mode)")
    parser.add_argument("--image_dir", help="Path to directory of input images (batch mode)")
    ...
    # Add silence option
    parser.add_argument("--silent", action="store_true", help="Silence standard output except for errors")
    
    args = parser.parse_args()
    
    
    # Configure logging based on silence option
    if args.silent:
    ...
    if not args.image and not args.image_dir:
        logger.error("Either --image or --image_dir must be provided")
        parser.print_help()
        sys.exit(1)
    ...
    if args.image:
        ...
        try:
            # Use silence context manager when processing
            with SilenceStdout(args.silent):
                result = process_image(
                    str(image_path), 
            ...
            # Print summary (this will show even in silent mode)
            print("\nProcessing complete!")
        ...
        if not image_dir.exists() or not image_dir.is_dir():
            logger.error(f"Image directory does not exist or is not a directory: {args.image_dir}")
            sys.exit(1)
            
        logger.info(f"Processing directory of images: {image_dir}")
        with SilenceStdout(args.silent):
        ...
        # Print summary
        print("\nProcessing complete!")
        print(f"Successfully processed {len(results['successful'])} images")
        print(f"Failed to process {len(results['failed'])} images")
        
        if results['successful']:
            print("\nSuccessfully processed images:")
            for item in results['successful']:
                img = item['image']
                # xml = item['output']['musicxml'] if item['output']['musicxml'] else "N/A"
                print(f"  {img} -> {xml}")
...
# python complete_pipeline.py --image /path/to/image.png --structure_model /path/to/structure.onnx --structure_mapping /path/to/structure_mapping.txt --symbol_model /path/to/symbol.pt --symbol_mapping /path/to/symbol_mapping.json

# Process all images in a directory:
# python complete_pipeline.py --image_dir /homes/es314/1-OMR_EVAL/GT/images --structure_model /path/to/structure.onnx --structure_mapping /path/to/structure_mapping.txt --symbol_model /path/to/symbol.pt --symbol_mapping /path/to/symbol_mapping.json

# Full example with all options:
# python complete_pipeline.py --image_dir /homes/es314/1-OMR_EVAL/GT/images --structure_model /homes/es314/omr-objdet-benchmark/scripts/encoding/clean_full_pipeline/onnx_models/may_2023_ex001/may_2023_ex001.onnx --structure_mapping /homes/es314/omr-objdet-benchmark/scripts/encoding/clean_full_pipeline/onnx_models/may_2023_ex001/mapping.txt --symbol_model /homes/es314/runs/detect/train/weights/best.pt --symbol_mapping /homes/es314/omr-objdet-benchmark/data/class_mapping.json --output_dir /homes/es314/1-OMR_EVAL/GT/output --conf 0.3 --iou 0.25 --high_conf 0.5 --overlap 0.5


# python /homes/es314/omr-objdet-benchmark/scripts/encoding/clean_full_pipeline/complete_pipeline_1/multi-image-run-pipeline.py --image_dir /import/c4dm-05/elona/EVAL/1-OMR_EVAL/GT/images \
...
#     --output_dir /homes/es314/omr-objdet-benchmark/scripts/encoding/clean_full_pipeline/complete_pipeline_1/fusion_detection_results \
#     --conf 0.3 \
#     --iou 0.25 \
#     --high_conf 0.5 \
#     --overlap 0.5 \
