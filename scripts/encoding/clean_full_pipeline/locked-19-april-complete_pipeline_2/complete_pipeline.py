...
#            "rest16th": "structure",    # FR-CNN is 1.11% better
#            "rest8th": "structure",     # FR-CNN is 24.24% better
#            "restquarter": "symbol",    # YOLO better for quarter rests
#            "rest32nd": "either"        # Both models equally good
#        }
       
#        # Define element-specific overlap thresholds
#        element_specific_thresholds = {
#            "restquarter": 0.01,  # Use a lower threshold for quarter rests to reduce duplicates
#            # Add other specific thresholds as needed
#        }
...
#        # Find quarter rest detections from both models
#        quarter_rest_map = {}
       
#        # First pass: collect all quarter rests and organize them by approximate position
#        for source, detections_dict in [("structure", structure_detections), ("symbol", symbol_detections)]:
#            for key, detections in detections_dict.items():
#                if "restquarter" in key:
#                    # For each quarter rest detection
#                    for detection in detections:
#                        bbox = detection.get("bbox", {})
#                        x = bbox.get("center_x", 0)
#                        y = bbox.get("center_y", 0)
                       
#                        # Create a position bucket (less precise than the compound key to help with matching)
#                        # For quarter rests specifically, use a larger grid size (e.g., 20 instead of 10)
#                        bucket_x = int(x / 20) * 20
#                        bucket_y = int(y / 20) * 20
#                        bucket = f"{bucket_x}_{bucket_y}"
                       
#                        if bucket not in quarter_rest_map:
#                            quarter_rest_map[bucket] = {"structure": [], "symbol": []}
                       
#                        quarter_rest_map[bucket][source].append((key, detection))
       
#        # Now, process quarter rests separately with custom overlap logic
#        processed_quarter_rest_keys = set()
       
#        # Get the specific overlap threshold for quarter rests
#        quarter_rest_threshold = element_specific_thresholds.get("restquarter", overlap_threshold)
       
#        # Process all quarter rest buckets
#        for bucket, sources in quarter_rest_map.items():
#            structure_quarters = sources["structure"]
#            symbol_quarters = sources["symbol"]
           
#            # If we have detections from both models in this vicinity
#            if structure_quarters and symbol_quarters:
#                # Find best matching pairs based on proximity
#                for struct_key, struct_detection in structure_quarters:
#                    struct_bbox = struct_detection.get("bbox", {})
#                    struct_x = struct_bbox.get("center_x", 0)
#                    struct_y = struct_bbox.get("center_y", 0)
                   
#                    # Find closest symbol model detection
#                    best_match = None
#                    best_distance = float('inf')
#                    best_symbol_key = None
                   
#                    for symbol_key, symbol_detection in symbol_quarters:
#                        symbol_bbox = symbol_detection.get("bbox", {})
#                        symbol_x = symbol_bbox.get("center_x", 0)
#                        symbol_y = symbol_bbox.get("center_y", 0)
                       
#                        # Calculate distance between detections
#                        distance = ((struct_x - symbol_x) ** 2 + (struct_y - symbol_y) ** 2) ** 0.5
                       
#                        # Calculate IoU between bounding boxes
#                        x1_max = max(struct_bbox.get("x1", 0), symbol_bbox.get("x1", 0))
#                        y1_max = max(struct_bbox.get("y1", 0), symbol_bbox.get("y1", 0))
#                        x2_min = min(struct_bbox.get("x2", 0), symbol_bbox.get("x2", 0))
#                        y2_min = min(struct_bbox.get("y2", 0), symbol_bbox.get("y2", 0))
                       
#                        # Check if boxes overlap
#                        if x1_max < x2_min and y1_max < y2_min:
#                            intersection_area = (x2_min - x1_max) * (y2_min - y1_max)
#                            struct_area = struct_bbox.get("width", 1) * struct_bbox.get("height", 1)
#                            symbol_area = symbol_bbox.get("width", 1) * symbol_bbox.get("height", 1)
#                            union_area = struct_area + symbol_area - intersection_area
#                            iou = intersection_area / union_area if union_area > 0 else 0
                           
#                            # Use quarter rest specific threshold
#                            if iou >= quarter_rest_threshold and distance < best_distance:
#                                best_match = symbol_detection
#                                best_distance = distance
#                                best_symbol_key = symbol_key
                   
#                    if best_match:
#                        # We found a match - use the symbol model detection (preferred for quarter rests)
#                        merged_elements.append(best_match)
#                        processed_quarter_rest_keys.add(struct_key)
#                        processed_quarter_rest_keys.add(best_symbol_key)
#                        logger.info(f"Merged quarter rest at bucket {bucket} (distance: {best_distance:.2f})")
           
#            # Add any symbol model quarter rests that weren't matched
#            for symbol_key, symbol_detection in symbol_quarters:
#                if symbol_key not in processed_quarter_rest_keys:
#                    merged_elements.append(symbol_detection)
#                    processed_quarter_rest_keys.add(symbol_key)
       
#        # Now process all other elements (non-quarter-rests)
#        for compound_key in all_compound_keys:
#            element_parts = compound_key.split('_', 1)
#            element_type = element_parts[0] if len(element_parts) > 0 else ""
#            position = element_parts[1] if len(element_parts) > 1 else ""
           
#            # Skip quarter rests as we've already processed them separately
#            if "restquarter" in element_type:
#                if compound_key in processed_quarter_rest_keys:
#                    continue
           
#            # Get the appropriate overlap threshold for this element type
#            current_overlap_threshold = element_specific_thresholds.get(element_type, overlap_threshold)
...
def process_image(image_path, structure_model_path, structure_mapping_path, 
    ...
    try:
        ...
        result = {
            ...
            "custom_xml": xml_results["custom_xml"],  # Add the custom XML path
    ...
    if image_path.is_file():
        ...
        for img_file in image_files:
            ...
            try:
                ...
                successful.append((str(img_file), result['musicxml']))
            except Exception as e:
...
# python /homes/es314/omr-objdet-benchmark/scripts/encoding/clean_full_pipeline/complete_pipeline_1/complete_pipeline.py  \
#         --image /homes/es314/omr-objdet-benchmark/scripts/encoding/clean_full_pipeline/complete_pipeline_1/tes_images/monophoic-test-score.png \
