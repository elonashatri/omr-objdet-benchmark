#/homes/es314/omr-objdet-benchmark/scripts/encoding/clean_full_pipeline/complete_pipeline_1/staff_line_detector.py
"""
...
class EnhancedStaffDetector:
    ...
    def _categorize_elements(self, detected_elements):
        """
        Categorize detected elements by type and perform de-duplication
        """
        ...
        # De-duplicate clefs specifically
        if len(categories["clefs"]) > 0:
            print(f"Found {len(categories['clefs'])} clefs, filtering duplicates...")
            filtered_clefs = []
            
            # Sort clefs by y position
            sorted_clefs = sorted(categories["clefs"], key=lambda c: c["bbox"]["center_y"])
            
            # Group clefs that are close in y-coordinate
            y_threshold = 0.05  # 5% of image height is a reasonable threshold
            current_group = [sorted_clefs[0]]
            
            for i in range(1, len(sorted_clefs)):
                curr_y = sorted_clefs[i]["bbox"]["center_y"] 
                prev_y = sorted_clefs[i-1]["bbox"]["center_y"]
                
                # If clef is close to previous one, add to current group
                if abs(curr_y - prev_y) < y_threshold:
                    current_group.append(sorted_clefs[i])
                else:
                    # Process the completed group
                    # Keep only the highest confidence clef from each group
                    if len(current_group) > 1:
                        # Find clef with highest confidence
                        best_clef = max(current_group, key=lambda c: c.get("confidence", 0))
                        filtered_clefs.append(best_clef)
                        print(f"  Keeping 1 of {len(current_group)} clefs at y={best_clef['bbox']['center_y']:.2f}")
                    else:
                        filtered_clefs.append(current_group[0])
                    
                    # Start a new group
                    current_group = [sorted_clefs[i]]
            ...
            # Replace the original clefs with filtered ones
            categories["clefs"] = filtered_clefs
            print(f"After filtering: {len(categories['clefs'])} unique clefs")
...
if __name__ == "__main__":
    # Example usage
    image_path = "/homes/es314/omr-objdet-benchmark/scripts/encoding/clean_full_pipeline/complete_pipeline_1/tes_images/2-solo-Christmas_Greeting-001.png"  # Replace with your image path
    detected_elements_path = "/homes/es314/omr-objdet-benchmark/scripts/encoding/clean_full_pipeline/complete_pipeline_1/results/2-solo-Christmas_Greeting-001/combined_detections/2-solo-Christmas_Greeting-001_combined_detections.json"  # Replace with your detections
    
    # Detect staff lines with pixel-perfect alignment
    staff_data = detect_staff_lines(
        image_path, 
        detected_elements_path,
        output_dir="/homes/es314/omr-objdet-benchmark/scripts/encoding/clean_full_pipeline/complete_pipeline_1/results/2-solo-Christmas_Greeting-001/staff_lines",
        pixel_perfect=True
    )
    
    # Alternative: Replace in existing pipeline
    # staff_data, staff_json = replace_in_pipeline(image_path, detected_elements_path)v