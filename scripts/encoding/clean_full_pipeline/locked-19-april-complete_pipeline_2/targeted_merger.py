...
def combine_detections_targeted(structure_detections, symbol_detections, high_conf_threshold=0.5, 
    ...
    rest_specialization = {
        ...
        "rest32nd": "either"         # Both are equally good
    }
    ...
    # 6. Get structural elements (beams, barlines, stems) - follows your original logic
    structure_elements = get_structure_elements(structure_non_rests)
    logger.info(f"Found {len(structure_elements)} structural elements (beams, barlines, stems)")
    
    # 7. Get high confidence structural elements from symbol model
    symbol_structure_elements = get_structure_elements(symbol_non_rests)
    ...
    for detection in symbol_non_rests:
        class_name = detection.get('class_name', '').lower()
    ...
    # 10. Combine all elements: kept rests + merged structural elements + non-structural elements
    combined_detections = {
        "detections": kept_rests + merged_structure_elements + non_structure_elements
    }
