...
class OMRProcessor:
    ...
    def group_notes_into_chords(self):
        ...
        with tall stems connecting them.
        """
        # Process each staff system separately
        for system in self.staff_systems:
            # Get all notes in this system
            system_notes = [n for n in system.elements if isinstance(n, Note)]
            ...
            avg_width = sum(n.width for n in system_notes) / len(system_notes) if system_notes else 20
            avg_height = sum(n.height for n in system_notes) / len(system_notes) if system_notes else 20
            staff_spacing = system.line_spacing if hasattr(system, 'line_spacing') and system.line_spacing else 10
            
            # STEP 1: Very aggressive first pass grouping based on horizontal proximity
            horizontal_tolerance = avg_width * 2.0  # Very aggressive
            
            # First, sort notes by x-position
            sorted_notes = sorted(system_notes, key=lambda n: n.x)
            
            # Identify potential chord groups using sliding window approach
            potential_chords = []
            
            # For each note, check if others are within horizontal tolerance
            for i, note in enumerate(sorted_notes):
                chord_group = [note]
                
                # Check all other notes for potential grouping
                for j, other_note in enumerate(sorted_notes):
                    if i == j:
                        continue
                    
                    # Very liberal horizontal matching
            ...
            # STEP 3: NEW - Special handling for vertically stacked adjacent notes (D4-E4 case)
            ...
            # This is crucial for catching notes connected by a tall stem like in your example
            for i, note1 in enumerate(system_notes):
                ...
                stem_group = [note1]
                for j, note2 in enumerate(system_notes):
                    if i == j or note2.is_chord_member:
                        continue
                        
                    # Check for vertical alignment (very tight horizontal tolerance)
    ...
    def _check_for_stem_sided_notes(self, notes_group, system):
        """
        Check for notes that appear on opposite sides of a stem.
        ...
        sorted_notes = sorted(notes_group, key=lambda n: (n.x, n.y))
        # Check for x-overlap and vertical alignment
        overlapping_groups = []
    ...
    def _detect_potential_stem_groups(self, notes, system):
        ...
        even when the stem itself isn't detected.
        ...
        Returns:
            List of lists, each containing notes likely connected by a stem
        """
        # Group notes that appear to be connected by a stem
        # (vertical alignment with appropriate spacing)
        potential_stem_groups = []
        for note in notes:
            # Look for notes vertically aligned with this one
            aligned_notes = [n for n in notes if 
    ...
    def _duration_based_barlines(self, system):
        ...
        Returns a list of x-positions and corresponding confidence scores.
        """
        # Get time signature (or default to 4/4)
        time_sig = system.time_signature
        if not time_sig:
            # Try to determine time signature from visible elements first
            time_sig = self._analyze_rhythmic_patterns(system)
            if not time_sig:
                time_sig = TimeSignature(beats=4, beat_type=4)
        ...
        # Calculate progressive note durations
        cumulative_duration = 0
        barline_positions = []
    ...
    def _create_barlines_at_positions(self, system, barline_positions, existing_barlines=None):
        ...
        filtered_positions = []
        for pos in barline_positions:
            # Check distance to nearest existing barline
            if all(abs(pos - ex_pos) > min_distance for ex_pos in existing_positions):
    ...
    def _analyze_barline_gaps(self, system, existing_barlines):
        ...
        Enhanced with content-aware validation.
        """
        if len(existing_barlines) < 2:
            return []
        ...
        if not musical_elements:
            return []  # No musical content to analyze
        ...
        if len(gaps) >= 3:
            ...
            # With few gaps, use mean of middle gaps (avoiding extremes)
            actual_gaps = [g for i, g in enumerate(gaps) 
                        if sum(1 for e in musical_elements if barline_xs[i] < e.x < barline_xs[i+1]) > 0]
            typical_gap = sum(actual_gaps) / len(actual_gaps) if actual_gaps else 100
        ...
        for i, gap in enumerate(gaps):
            ...
            if gap > typical_gap * 1.5 and elements_in_gap >= 3:  # Gap must have enough content
                ...
                # Instead of evenly dividing, look for natural breaks in musical content
                if missing_count >= 1:
                    # Group notes into clusters to find natural divisions
                    element_positions = [e.x for e in musical_elements 
                                    if barline_xs[i] < e.x < barline_xs[i+1]]
        ...
        for idx, gap_size, start_pos, end_pos, gap_type in boundary_gaps:
            # Count musical elements in this boundary gap
            elements_in_gap = sum(1 for e in musical_elements 
                                if start_pos < e.x < end_pos)
            
            # Skip gaps with insufficient content
