...
def load_faster_rcnn_class_names(class_mapping_file):
    """Load class names from Faster R-CNN mapping file"""
    class_names = {}
    print(f"Reading class mapping from: {class_mapping_file}")
    try:
        with open(class_mapping_file, 'r') as f:
            line_count = 0
            for line in f:
                ...
                if ":" in line:
                    ...
                    if len(parts) == 2:
                        ...
                        try:
                            class_id = int(class_id) - 1  # Convert to 0-indexed for consistency
                            class_names[class_id] = class_name
                            if line_count <= 5:  # Print first few classes as a sample
                                print(f"  Sample class: ID {class_id} = {class_name}")
                        except ValueError as ve:
                            print(f"  Warning: Invalid class ID in line {line_count}: {parts[0]} - {ve}")
        
        print(f"Loaded {len(class_names)} classes from Faster R-CNN mapping")
        if len(class_names) == 0:
            print("WARNING: No classes were loaded from the mapping file!")
            print("First 10 lines of the file for debugging:")
            with open(class_mapping_file, 'r') as f:
...
def plot_detections(
    ...
    conf_threshold=0.25,
    output_path=None,
    max_detections=600,  # Updated default to 600
    plot_labels=True,
    random_colors=False,
    ...
    if hasattr(results, 'boxes') and hasattr(results.boxes, 'xyxy') and results.boxes.xyxy.shape[0] > 0:
...
def apply_custom_nms(boxes, scores, class_ids, iou_threshold=0.35, score_threshold=0.25, max_detections=600):
    """
...
def detect_faster_rcnn(model_path, image_path, class_names, output_dir, img_stem, 
                      conf_threshold=0.25, max_detections=600, iou_threshold=0.35):
    """
...
def detect_music_notation(model_path, image_path, class_mapping_file, 
                         output_dir="results", conf_threshold=0.25, 
                         max_detections=600, iou_threshold=0.35):
    ...
    # Check if this is a Faster R-CNN model based on the path
    is_faster_rcnn = any([
    ...
    print(f"Detected model type: {'Faster R-CNN' if is_faster_rcnn else 'YOLO'}")
    ...
    # If it's a Faster R-CNN model, use our placeholder generator
    if is_faster_rcnn:
...
def main():
    ...
    parser.add_argument("--max_detections", type=int, default=600, help="Maximum number of detections to display")
